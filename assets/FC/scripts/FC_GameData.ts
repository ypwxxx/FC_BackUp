...
export default class FC_GameData {
    ...
    // 设置开始游戏信息
    public setStartInfo(planeType: PLANE_TYPE[], playerType: PLAYER_TYPE[], launchNum: number){
        ...
        let temp = [];
        for(let i = 0; i < GAME_BASE_DATA.player_icon_count; i++){
            temp.push(i);
        }
        ...
        if(launchNum <= 2 && launchNum >= 0){
            this._launchPlaneNum = GAME_BASE_DATA.launch_plane_num[launchNum];
        }
    };
    ...
    // 获取玩家顺序
    public getPlayerOrder(type: PLANE_TYPE): number{
        let results = null;
        if(type === PLANE_TYPE.THE_RED){
            results = 0;
        }else if(type === PLANE_TYPE.THE_YELLOW){
