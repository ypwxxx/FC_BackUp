...
export default class Comm_Command {
    public constructor(msg: string, content: any = null) {
        this._msg = msg;
    ...
    private _setDefaultFailError(num: number, other: string = ''){
        ...
        if(num == 1){
            this._failError = {
                code: -1,
                msg: `${this._msg}: ${Comm_Command._FailError_1} - ${other}`
            };
        }else if(num == 2){
            this._failError = {
                code: -1,
                msg: `${this._msg}: ${Comm_Command._FailError_2} - ${other}`
            };
        }
    };
